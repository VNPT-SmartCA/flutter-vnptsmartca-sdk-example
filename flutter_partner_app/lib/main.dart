  ...
  initMethod(BuildContext context) {
    platform.setMethodCallHandler((call) async {
      if (call.method == "getAuthenticationResult") {
        ...
        if (call.arguments is Map && call.arguments["status"] == 0) {
          // Xử lý khi thành công
          showDialogMessage(context, "Giao dịch thành công: ${call.arguments["statusDesc"]}");
        } else {
          // Xử lý khi thất bại
          showDialogMessage(context, "Giao dịch thất bại: ${call.arguments["statusDesc"]}");
        }
      }
    });
  }
    ...
    return Scaffold(
      ...
      body: Center(
        ...
        child: Column(
          ...
          children: <Widget>[
            ElevatedButton(onPressed: getAuthentication, child: Text("getAuthentication")),
            SizedBox(height: 30),
            ElevatedButton(onPressed: toMainPage, child: Text("getMainPage")),
            SizedBox(height: 30),
            ElevatedButton(onPressed: getWaitingTransaction, child: Text("getWaitingTransaction"))
          ],
        ),
      ),
    );
